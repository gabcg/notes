...
Simulations come from femto to micro. In small systems, the mili can be done with small stuff (bigger needs a supercomputer).

For the second we can wait till tech advances, or use advanced algorithms.

**Prot vibr**

Bond vibr happens in pico-to nano. Helix mov happens in mili to a second.

Conformation is a snapshot in a single point in time. Over the time, the prot can addopt some confor, and some cluster in zones of similar conformations:

* the conformation state
* Intermediate
* Inactive

It is important to use experimental data:

* Crystallography: good resolution. Shows the average of the conformation state cluster.
* Spectroscopy (NMR/fluorescence): small proteins well in nmr, big proteins dont allow such a good resolution, but it gives different structures (even if you just can see the changes of the probe).

## Molecular mechanics

It is a method to calculate geometries and energies. It is a simplified system to be able to do this calculations with larger molecules.

**Stretching energy**

Sums the energy of each bond.

r0 comes from experiments (nature, crystallography, ab initio blabla) or quantum mechanicals calculations. Collected in a file called the force field.

kb: the higher, the stiffer.

If the difference in the parenthesis is high, then the energy goes up.
...
Parameters for each 4 atoms (how a part rotates in reference to the other).

The function is like that because we can keep rotating.

In the example, it is not very preferred and A would be high.

This term corrects for the others, I think.

**Non-bonded energy**

* Comb of 2 contributions: van der waals and electrostatic.
* VDW: 6-12 potential. Dependency is at the power of 6.
	* A defines stickiness. Minus value does something (ask).
	* Repultion: positive sign for B.
	* A and B param are in the force field file. Soft atoms can be deformed when they are close, hard cant.
* Electro term:
	* r are radius (always).
...
* Some are specialised for some problems.
* Over the years evolve: we can simulate longer, which allows to see problems and there is need to correct the force fields.
* We see the mass of each atom. Different types of H atoms.
* Then, it defines the bonds.
* Then, two types of atoms, spring contant
* Then it comes a section for angles. Spring constant, reference... LAter it has comments on where it was taken from.
